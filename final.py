...
brush = 5
eraser = 50
questions=["wdsbxfnc","arhaan goat","Hello"]
...
# Replace finger detection with keyboard input
        ...
        if fingers[1] and fingers[2]:
            ...
            if x1 < 100:  # Detect clicks in the left-side header
                if 400 < y1 < 450:
                    ...
                    drawColor = (0,0,139)  # Deep blue color
        ...
        if key == ord('s'):
            ...
            response = model.generate_content([uploaded_file, "what do you seen in the blue coloured line in one word just one no matter what "])
            final = response.text
            print(final)
        # 5. If drawing mode - Index finger is up
        elif fingers[1] and not fingers[2] and 700 < x1 < 1200 and 50 < y1 < 600:
            # Display mode
            cv2.putText(img, "DRAW MODE", (20, 680), cv2.FONT_HERSHEY_SIMPLEX, 0.6, WHITE, 1, cv2.LINE_AA)  # White text
